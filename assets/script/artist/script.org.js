...
export default {
	...
	computed: {
		...
		init() {
			...
			// console.log(artist)
			this.albums = this.$.all.album.filter(album =>
				album.tk.some(
					// track => track.ar.indexOf(this.artistName) >= 0
					track => track.a.find(e => e == this.artist.id)
				)
			);

			this.tracks = this.albums
				.map(album =>
					album.tk.filter(track => track.a.find(e => e == this.artist.id))
				)
				.reduce((prev, next) => prev.concat(next), [])
				.sort((a, b) => (a.p < b.p ? 1 : -1));

			var artRed = this.albums
				.map(album =>
					album.tk
						.map(track => track.a)
						.reduce((prev, next) => prev.concat(next), [])
			...
			this.artistRelated = this.artistRelatedIndex
				.filter(this.$.arrayComparer(this.artistRecommendedIndex))
				.map(i => this.$.all.artist.find(e => e.id == i))
				.sort((a, b) => (a.plays < b.plays ? 1 : -1))
				.map(
					e =>
						(this.$.utf8(this.artistName) ||
							this.artist.lang.find(e => e == 2)) &&
						e.aka
							? e.aka
							: e.name
			...
			this.artistRecommended = this.artistRecommendedIndex
				.map(i => this.$.all.artist.find(e => e.id == i))
				.sort((a, b) => (a.plays < b.plays ? 1 : -1))
				.map(e => (this.$.utf8(this.artistName) && e.aka ? e.aka : e.name));
		...
		artistCategory() {
			return this.$.artistAlphabetically(
				artist =>
					artist.id > 1 &&
					artist.lang.find(e => (this.activeLang ? e == this.activeLang : true))
