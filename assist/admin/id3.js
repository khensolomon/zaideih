...
async function processor() {
	...
	const taskMain = store.bucket.data
		.filter(
			(e) =>
				(job.params.albumId && job.params.albumId == e.id) ||
				(!e.track.length && !job.params.albumId)
		)
		.map((e) => {
			e.task = [];
			e.track = [];
	...
	var scan = async function (active) {
		if (active.task.length) {
			...
			if (file.split(".").pop().toLowerCase() == "mp3") {
				...
				await job
					.reader([active.dir, file].join("/"))
					.then((tag) => active.track.push(tag.tmp))
...
async function readCloudID3(file = "") {
	return await cloud.bucket
		.file(file)
		.download()
		.then((buffer) => readId3(buffer[0], file))
		.catch((e) => {
			throw e;
		});
}
...
async function readLocalID3(file = "") {
	...
	return await fs.promises
		.readFile(file)
		.then((buffer) => readId3(buffer, file))
		.catch((e) => {
			throw e;
		});
}
	...
	try {
		...
		if (tag.genre)
			meta.genre = tag.genre
				...
				.split(/(?:,|;)+/)
				.map((s) => s.trim());
		if (tag.performerInfo)
			meta.albumartist = tag.performerInfo
				.toString()
				.split(/(?:,|;)+/)
				.map((s) => s.trim());
