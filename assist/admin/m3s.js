		...
		.then((res) => {
			...
			try {
				var html = dom.window.document.querySelectorAll(
					"div#Albums>table>tbody>tr"
				);
				// console.log(Object.keys(html).length === 0);
				var nextPage =
					dom.window.document.querySelectorAll("div.t-pager > a")[2];
				...
				if (html.length) {
					html.forEach((e) => {
						...
						var rowCheck =
							setting.info.albumListData.find((e) => e.id == rowId) ==
							undefined;
						if (rowCheck) {
	...
	await ask
		.get({
			...
			path: "/mm/artists/list?artType=artistListType&Artists-page=artistListPage"
				...
				.replace(
					"artistListPage",
					setting.taskCurrent.artistListPage.toString()
				),
			method: "GET",
			headers: {
				referer: domainName,
		...
		.then((res) => {
			...
			try {
				...
				html.forEach((e) => {
					...
					var rowCheck =
						setting.info.artistListData.find((e) => e.id == rowId) == undefined;
					if (rowCheck) {
...
job.albumDetailProcess = async function (albumDetailTmp) {
	...
	if (albumDetailTmp.local && albumDetailTmp.local > 0) {
		...
		var urlOptions = {
			host: domainName,
			path: "/mm/albums/pull/albumId/fe".replace(
				"albumId",
				setting.taskCurrent.albumId.toString()
			),
			method: "GET",
			port: 443,
			headers: {
		...
		await ask
			...
			.then(async (res) => {
				...
				try {
					...
					var albumArt = albumWrap
						.querySelector("div.albumimg>img")
						.getAttribute("src");
					var albumName = albumWrap
						.querySelector("div.albumdesc>div#albuminfo>div.h1m")
						.textContent.trim();

					albumWrap
						.querySelectorAll("div.albumdesc>div#albuminfo>div.descbox>ul>li")
						.forEach((e, i) => {
					...
					dom.window.document
						.querySelectorAll("#GridTracks>table>tbody>tr")
						.forEach(async (e, i) => {
							var trkNumber = i + 1;
...
job.artistAlbum = async function () {
	...
	var tmpTask = setting.info.artistListData.filter(
		(e) => !e.local || e.local < 1
	);
		...
		if (album.info) {
			...
			for (const track of album.track) {
				if (track.local && track.local > 0) {
					var file = "d:/m3s/albumId/trackNumber.mp3"
						.replace("albumId", album.id)
						.replace("trackNumber", track.number);
					let tags = {
						TYER: year,
						TCON: "",
...
async function trackAudioDownload() {
	...
	var urlOptions = {
		host: domainName,
		path: "/track/ProcessNTrack/trackId".replace(
			"trackId",
			setting.taskCurrent.track.id
		),
	...
	var file = setting.info.trackFile
		.replace("albumId", setting.taskCurrent.albumId.toString())
		.replace("trackNumber", setting.taskCurrent.track.number);
...
async function albumArtDownload() {
	...
	var fileExtension = setting.taskCurrent.albumArt
		.split(".")
		.pop()
		.toLowerCase();
	var file = setting.info.albumArtFile
		.replace("albumId", setting.taskCurrent.albumId.toString())
